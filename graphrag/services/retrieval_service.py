    ...
    async def hybrid_search(
        ...
        *,
        ...
        for hop in range(1, expand_hops + 1):
            ...
            # Decay by hop; use best seed score as base if available
...
def pack_context(hits: list[SearchHit], token_budget: int) -> str:
    ...
    for hit in hits:
        ...
        # Mild diversity preference: allow first chunk per node freely, later ones if room
        extra_cost = 0 if hit.node_id not in seen_nodes else 0
        cost = len(block) + extra_cost
